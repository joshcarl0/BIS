...
/**
 * Compatibility layer:
 * Support both old and new controller variable names.
 */
$todayRows  = $todayRows ?? ($releasedTodayList ?? []);
$recentLogs = $recentLogs ?? ($recentActivities ?? ($activityLogs ?? []));

// ensure arrays
$todayRows  = is_array($todayRows) ? $todayRows : [];
$recentLogs = is_array($recentLogs) ? $recentLogs : [];

// limit recent logs to 5 (kahit 10 pa pinasa)
$recentLogs = array_slice($recentLogs, 0, 5);

/**
 * Summary: count only (removed "Collected" display)
 */
$releasedTodaySummary = $releasedTodaySummary ?? [
...
function formatDashboardDateTime($dt): string {
    if (!$dt) return '';
    $ts = strtotime($dt);
...
<body style="background:#D6D5D7;">
                    ...
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                            ...
                            <small class="text-muted">
                                Count: <b><?= (int)($releasedTodaySummary['total_count'] ?? 0) ?></b>
                            </small>
                        </div>
                        ...
                        <table class="table table-sm align-middle">
                            ...
                            <?php else: foreach ($todayRows as $r): ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($r['document_type'] ?? ($r['document_name'] ?? '')) ?></td>
                                    <td class="text-end">₱<?= number_format((float)($r['amount_paid'] ?? 0), 2) ?></td>
                                    <td><?= htmlspecialchars(formatDashboardDateTime($r['released_at'] ?? '')) ?></td>
                                </tr>
                    ...
                    <div class="table-responsive">
                        <table class="table table-sm align-middle">
                            ...
                            <?php else: foreach ($recentLogs as $log): ?>
                                <tr>
                                    <td class="text-nowrap"><?= htmlspecialchars(formatDashboardDateTime($log['created_at'] ?? '')) ?></td>
                                    <td class="text-nowrap"><?= htmlspecialchars($log['action'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($log['description'] ?? '') ?></td>
                                    <td class="text-nowrap"><?= htmlspecialchars($log['actor_role'] ?? ($log['actor'] ?? 'system')) ?></td>
                                </tr>
                            <?php endforeach; endif; ?>
                            </tbody>
                        </table>
    ...
    <script>
        ...
        if (statusCanvas && statusLabels.length && statusCounts.length) {
